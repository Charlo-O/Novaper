...
function inferDecisionProvider(
  config: Partial<ConfigSaveRequest> | null | undefined
) {
  if (!config) {
    return DECISION_PRESETS[0].name;
  }
  if (config.decision_provider) {
    return config.decision_provider;
  }
    ...
    const loadConfiguration = async () => {
      try {
        ...
        const decisionProvider =
          data.decision_provider || inferDecisionProvider(data);
        const usesCodeX =
        ...
        if (
          useDefaultConfig ||
          (usesCodeX &&
            !auth?.providers.codexOAuth.authenticated)
        ) {
          setShowConfig(true);
        }
      } catch (err) {
    ...
    <div className="h-full flex relative min-h-0">
      ...
      <Dialog open={showConfig} onOpenChange={setShowConfig}>
        <DialogContent className="sm:max-w-md h-[75vh] flex flex-col">
          ...
          {false && (
            <div className="flex-shrink-0 rounded-xl border border-sky-200 bg-sky-50 px-3 py-3 text-sm text-sky-950 dark:border-sky-900 dark:bg-sky-950/30 dark:text-sky-100">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-2 min-w-0">
                <ShieldCheck className="mt-0.5 h-4 w-4 flex-shrink-0" />
                <div className="min-w-0">
                  <div className="font-medium">Codex OAuth</div>
                  <div className="text-xs text-sky-800 dark:text-sky-200 break-all">
                    {codexAuthenticated
                      ? `已认证，可直接调用模型${codexAuth?.accountId ? `：${codexAuth?.accountId}` : ''}`
                      : '未认证。视觉模型和决策模型会统一通过 Codex OAuth 调用。'}
                  </div>
                  {codexAuth?.error && (
              ...
              <Button
                type="button"
                variant={codexAuthenticated ? 'outline' : 'twitter'}
                size="sm"
                className="shrink-0"
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="model_name"
                  value={
                    tempConfig.model_name || (isVisionCodeX ? CODEX_DEFAULT_MODEL : '')
                  }
                  onChange={e =>
                    setTempConfig({
                      ...tempConfig,
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {availableAgentPresets.map(preset => (
                    <button
                      key={preset.name}
                      type="button"
                      onClick={() =>
              ...
              onClick={() => {
                ...
                if (config) {
                  const visionProvider =
                    config.vision_provider || inferVisionProvider(config);
                  const decisionProvider =
                    config.decision_provider || inferDecisionProvider(config);
                  const usesCodeX =
                    visionProvider === CODEX_PRESET_NAME ||
                    decisionProvider === CODEX_PRESET_NAME;
                  setTempConfig({
                    ...
                    agent_type: usesCodeX
                      ? CODEX_AGENT_NAME
                      : config.agent_type || 'glm-async',
